import '../styles/globals.css';
export { default as i18n } from '../i18n';
export { default as useLocale } from '../components/hooks/useLocale';
export { default as HomePage } from '../pages/index';
export { ReportPage as ReportPageComponent1 } from '../pages/reports/index';
export { ReportPage as ReportPageComponent2 } from '../pages/reports/reports';
export { ReportPage as ReportPageComponent3 } from '../pages/reports/test';
export { default as TestPage } from '../pages/reports/mew';
